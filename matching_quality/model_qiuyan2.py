...
class Topic_Model2:
    ...
    def vectorize(self, sentences, token_lists, method=None):
        ...
        # else:
            vec_lda2 = self.vectorize(sentences, token_lists, method='LDA')
            # lda = pd.DataFrame(vec_lda2)
            # lda.to_csv('./vector/lda_vector.csv')

            vec_mpnet2 = self.vectorize(sentences, token_lists, method='MPNet')
            # mpnet = pd.DataFrame(vec_mpnet2)
            # mpnet.to_csv('./vector/mpnet_vector.csv')

            #LDA和BERT向量连接
            vec_ldampnet2 = np.c_[vec_lda2, vec_mpnet2]
            # ldampnet = pd.DataFrame(vec_ldampnet2)
            # ldampnet.to_csv('./vector/ldampnet_vector.csv')
            return vec_ldampnet2

            # self.vec['LDA_BERT_FULL'] = vec_ldampnet2
            # if not self.AE:
            #     self.AE = Autoencoder()
            #     print('Fitting Autoencoder ...')
            #     self.AE.fit(vec_ldampnet2)
            #     print('Fitting Autoencoder Done!')
            # vec2 = self.AE.encoder.predict(vec_ldampnet2)
            # return vec2
